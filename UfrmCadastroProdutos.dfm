object frmCadastroProdutos: TfrmCadastroProdutos
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu]
  Caption = 'Cadastro de Produtos'
  ClientHeight = 622
  ClientWidth = 648
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 8
    Top = 440
    Width = 31
    Height = 13
    Caption = 'Label1'
  end
  object btnCancelar: TSpeedButton
    Left = 549
    Top = 208
    Width = 33
    Height = 45
    Enabled = False
    Glyph.Data = {
      }
    OnClick = btnCancelarClick
  end
  object btnEditar: TSpeedButton
    Left = 471
    Top = 208
    Width = 33
    Height = 45
    Enabled = False
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000}
    OnClick = btnEditarClick
  end
  object btnExcluir: TSpeedButton
    Left = 510
    Top = 208
    Width = 33
    Height = 45
    Enabled = False
    Glyph.Data = {
      
      FFFFFFFFFF00}
    OnClick = btnExcluirClick
  end
  object btnSave: TSpeedButton
    Left = 588
    Top = 208
    Width = 33
    Height = 45
    Enabled = False
    Glyph.Data = {
      }
    OnClick = btnSaveClick
  end
  object btnNovo: TSpeedButton
    Left = 432
    Top = 208
    Width = 33
    Height = 45
    Glyph.Data = {
      }
    OnClick = btnNovoClick
  end
  object Label2: TLabel
    Left = 195
    Top = 20
    Width = 28
    Height = 16
    Caption = 'SKU:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label3: TLabel
    Left = 447
    Top = 20
    Width = 28
    Height = 16
    Caption = 'EAN:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label4: TLabel
    Left = 19
    Top = 61
    Width = 73
    Height = 16
    Caption = 'DESCRICAO:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label5: TLabel
    Left = 29
    Top = 102
    Width = 63
    Height = 16
    Caption = 'PESO (kg):'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label6: TLabel
    Left = 369
    Top = 113
    Width = 106
    Height = 16
    Caption = 'DATA CADASTRO:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label9: TLabel
    Left = 403
    Top = 61
    Width = 72
    Height = 16
    Caption = 'TIPO BAIXA:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label10: TLabel
    Left = 47
    Top = 20
    Width = 44
    Height = 16
    Caption = 'Codigo:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object dbGridCadProduto: TDBGrid
    Left = 0
    Top = 259
    Width = 648
    Height = 363
    Align = alBottom
    DrawingStyle = gdsGradient
    Options = [dgTitles, dgIndicator, dgColumnResize, dgRowLines, dgTabs, dgRowSelect, dgMultiSelect, dgTitleClick, dgTitleHotTrack]
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnCellClick = dbGridCadProdutoCellClick
  end
  object GroupBox1: TGroupBox
    Left = 8
    Top = 188
    Width = 393
    Height = 65
    Caption = 'Pesquisar'
    TabOrder = 1
    object btnPesquisa: TSpeedButton
      Left = 346
      Top = 16
      Width = 44
      Height = 46
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF000000}
      OnClick = btnPesquisaClick
    end
    object txtPesquisa: TEdit
      Left = 3
      Top = 24
      Width = 217
      Height = 21
      TabOrder = 0
    end
    object rbSku: TRadioButton
      Left = 248
      Top = 16
      Width = 57
      Height = 17
      Caption = 'sku'
      Checked = True
      TabOrder = 1
      TabStop = True
    end
    object rbDescricao: TRadioButton
      Left = 248
      Top = 39
      Width = 73
      Height = 17
      Caption = 'Descricao'
      TabOrder = 2
    end
  end
  object txtDescricao: TEdit
    Tag = 100
    Left = 98
    Top = 60
    Width = 271
    Height = 21
    Enabled = False
    MaxLength = 100
    TabOrder = 2
  end
  object txtPeso: TEdit
    Left = 97
    Top = 101
    Width = 72
    Height = 21
    Enabled = False
    MaxLength = 5
    ParentShowHint = False
    ShowHint = True
    TabOrder = 3
    TextHint = '0,000'
  end
  object pickerDtCadastro: TCalendarPicker
    Left = 481
    Top = 102
    Height = 32
    CalendarHeaderInfo.DaysOfWeekFont.Charset = DEFAULT_CHARSET
    CalendarHeaderInfo.DaysOfWeekFont.Color = clWindowText
    CalendarHeaderInfo.DaysOfWeekFont.Height = -13
    CalendarHeaderInfo.DaysOfWeekFont.Name = 'Segoe UI'
    CalendarHeaderInfo.DaysOfWeekFont.Style = []
    CalendarHeaderInfo.Font.Charset = DEFAULT_CHARSET
    CalendarHeaderInfo.Font.Color = clWindowText
    CalendarHeaderInfo.Font.Height = -20
    CalendarHeaderInfo.Font.Name = 'Segoe UI'
    CalendarHeaderInfo.Font.Style = []
    Color = clWindow
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGray
    Font.Height = -16
    Font.Name = 'Segoe UI'
    Font.Style = []
    ParentFont = False
    TabOrder = 4
    TextHint = 'selecione'
  end
  object txtEan: TEdit
    Left = 481
    Top = 19
    Width = 140
    Height = 21
    Enabled = False
    MaxLength = 13
    TabOrder = 5
  end
  object txtSku: TEdit
    Left = 229
    Top = 19
    Width = 140
    Height = 21
    Enabled = False
    MaxLength = 15
    TabOrder = 6
  end
  object cboTipoBaixa: TComboBox
    Left = 481
    Top = 60
    Width = 140
    Height = 21
    Enabled = False
    TabOrder = 7
    Text = 'Selecione...'
    Items.Strings = (
      'FIFO'
      'LIFO'
      'FEFO')
  end
  object txtCodigo: TEdit
    Left = 97
    Top = 19
    Width = 72
    Height = 21
    Enabled = False
    TabOrder = 8
  end
  object pgConnection: TFDConnection
    Params.Strings = (
      'Database=app_wms'
      'User_Name=postgres'
      'Password=q1w2e3r4'
      'Server='
      'DriverID=PG')
    Connected = True
    LoginPrompt = False
    Left = 552
    Top = 148
  end
end
