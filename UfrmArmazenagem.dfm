object frmArmazenagem: TfrmArmazenagem
  Left = 0
  Top = 0
  Caption = 'Armazenar Produto'
  ClientHeight = 693
  ClientWidth = 901
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 62
    Top = 16
    Width = 37
    Height = 13
    Caption = 'Codigo:'
  end
  object Label2: TLabel
    Left = 502
    Top = 19
    Width = 68
    Height = 13
    Caption = 'Data Entrada:'
  end
  object Label3: TLabel
    Left = 27
    Top = 61
    Width = 72
    Height = 13
    Caption = 'Produto (SKU):'
  end
  object Label6: TLabel
    Left = 280
    Top = 16
    Width = 60
    Height = 13
    Caption = 'Quantidade:'
  end
  object Label7: TLabel
    Left = 50
    Top = 117
    Width = 49
    Height = 13
    Caption = 'Endereco:'
  end
  object btnCancelar: TSpeedButton
    Left = 821
    Top = 205
    Width = 33
    Height = 45
    Enabled = False
    Glyph.Data = {
      }
    OnClick = btnCancelarClick
  end
  object btnEditar: TSpeedButton
    Left = 745
    Top = 205
    Width = 33
    Height = 45
    Enabled = False
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000}
    OnClick = btnEditarClick
  end
  object btnExcluir: TSpeedButton
    Left = 784
    Top = 205
    Width = 33
    Height = 45
    Enabled = False
    Glyph.Data = {
      
      FFFFFFFFFF00}
    OnClick = btnExcluirClick
  end
  object btnSave: TSpeedButton
    Left = 860
    Top = 205
    Width = 33
    Height = 45
    Enabled = False
    Glyph.Data = {
      }
    OnClick = btnSaveClick
  end
  object btnNovo: TSpeedButton
    Left = 706
    Top = 205
    Width = 33
    Height = 45
    Glyph.Data = {
      }
    OnClick = btnNovoClick
  end
  object Label4: TLabel
    Left = 482
    Top = 64
    Width = 85
    Height = 13
    Caption = 'Data Vencimento:'
  end
  object dbGridArmazenagem: TDBGrid
    Left = 0
    Top = 256
    Width = 901
    Height = 437
    Align = alBottom
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnCellClick = dbGridArmazenagemCellClick
  end
  object txtCodigo: TEdit
    Left = 105
    Top = 13
    Width = 49
    Height = 21
    Enabled = False
    TabOrder = 1
  end
  object txtProduto: TEdit
    Left = 105
    Top = 58
    Width = 113
    Height = 21
    Enabled = False
    ReadOnly = True
    TabOrder = 2
  end
  object btnPesquisarProduto: TButton
    Left = 224
    Top = 56
    Width = 26
    Height = 25
    Caption = '...'
    Enabled = False
    TabOrder = 3
    OnClick = btnPesquisarProdutoClick
  end
  object txtEndereco: TEdit
    Left = 105
    Top = 114
    Width = 113
    Height = 21
    Enabled = False
    ReadOnly = True
    TabOrder = 4
  end
  object btnPesquisarEnde: TButton
    Left = 224
    Top = 112
    Width = 26
    Height = 25
    Caption = '...'
    Enabled = False
    TabOrder = 5
    OnClick = btnPesquisarEndeClick
  end
  object txtQuantidade: TEdit
    Left = 346
    Top = 13
    Width = 63
    Height = 21
    Enabled = False
    NumbersOnly = True
    TabOrder = 6
  end
  object pickerDataEntrada: TCalendarPicker
    Left = 573
    Top = 11
    Height = 32
    CalendarHeaderInfo.DaysOfWeekFont.Charset = DEFAULT_CHARSET
    CalendarHeaderInfo.DaysOfWeekFont.Color = clWindowText
    CalendarHeaderInfo.DaysOfWeekFont.Height = -13
    CalendarHeaderInfo.DaysOfWeekFont.Name = 'Segoe UI'
    CalendarHeaderInfo.DaysOfWeekFont.Style = []
    CalendarHeaderInfo.Font.Charset = DEFAULT_CHARSET
    CalendarHeaderInfo.Font.Color = clWindowText
    CalendarHeaderInfo.Font.Height = -20
    CalendarHeaderInfo.Font.Name = 'Segoe UI'
    CalendarHeaderInfo.Font.Style = []
    Color = clWindow
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGray
    Font.Height = -16
    Font.Name = 'Segoe UI'
    Font.Style = []
    ParentFont = False
    TabOrder = 7
    TextHint = 'seleciona'
  end
  object pickerDataVencimento: TCalendarPicker
    Left = 573
    Top = 56
    Height = 32
    CalendarHeaderInfo.DaysOfWeekFont.Charset = DEFAULT_CHARSET
    CalendarHeaderInfo.DaysOfWeekFont.Color = clWindowText
    CalendarHeaderInfo.DaysOfWeekFont.Height = -13
    CalendarHeaderInfo.DaysOfWeekFont.Name = 'Segoe UI'
    CalendarHeaderInfo.DaysOfWeekFont.Style = []
    CalendarHeaderInfo.Font.Charset = DEFAULT_CHARSET
    CalendarHeaderInfo.Font.Color = clWindowText
    CalendarHeaderInfo.Font.Height = -20
    CalendarHeaderInfo.Font.Name = 'Segoe UI'
    CalendarHeaderInfo.Font.Style = []
    Color = clWindow
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGray
    Font.Height = -16
    Font.Name = 'Segoe UI'
    Font.Style = []
    ParentFont = False
    TabOrder = 8
    TextHint = 'seleciona '
  end
  object txtDescProduto: TEdit
    Left = 265
    Top = 58
    Width = 179
    Height = 21
    Enabled = False
    ReadOnly = True
    TabOrder = 9
  end
  object txtEtiqueta: TEdit
    Left = 265
    Top = 114
    Width = 179
    Height = 21
    Enabled = False
    ReadOnly = True
    TabOrder = 10
  end
  object GroupBox1: TGroupBox
    Left = 27
    Top = 184
    Width = 302
    Height = 66
    Caption = 'Pesquisar'
    TabOrder = 11
    object btnPesquisa: TSpeedButton
      Left = 250
      Top = 24
      Width = 44
      Height = 31
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF000000}
      OnClick = btnPesquisaClick
    end
    object txtPesquisa: TEdit
      Left = 9
      Top = 26
      Width = 235
      Height = 24
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      TextHint = 'Numero ou Descricao'
    end
  end
  object pgConnection: TFDConnection
    Params.Strings = (
      'Database=app_wms'
      'User_Name=postgres'
      'Password=q1w2e3r4'
      'Server=localhost'
      'DriverID=PG')
    Connected = True
    LoginPrompt = False
    Left = 848
    Top = 40
  end
end
